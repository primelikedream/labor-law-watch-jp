...
async function main() {
  console.log("収集開始...");
  const [mhlwItems, egovItems, nikkeiItems] = await Promise.all([
  ...
  console.log(`MHLW: ${mhlwItems.length}件 / e-Gov: ${egovItems.length}件 / 日経: ${nikkeiItems.length}件 取得`);

  const data = await loadData();
  const { merged, addedCount } = mergeItems(data.items, [...mhlwItems, ...egovItems, ...nikkeiItems]);
