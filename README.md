...
- **関連性フィルタ**([src/relevance.ts](src/relevance.ts)): 日経・労政時報はGoogleニュースのキーワード検索で拾っているため、求人広告そのもの・海外の雇用統計・上場企業の適時開示・地域の企業誘致ニュースなど、労働法規/人事施策と直接関係のない項目が混じることがある。Claudeに「日本の労働関連法規」または「日本企業の人事制度の取組み紹介」に該当するかを判定させ、対象外の項目はストアから除外する。判定済みの項目は再判定しない。未設定時はキーワード一致のみで判定される(除外は行われない)。
...
- **法令改正の要約強化**([src/summarize.ts](src/summarize.ts)): e-Govの法令改正情報は法令名・改正名・施行日しか分からず、本文(何がどう変わるか)はAPIから取得できない。同じ出来事(storyId)を厚労省・日経・労政時報が合わせて報じている場合は、それらの見出しも要約の材料として渡し、実在する情報の範囲でより具体的な要約にする(架空の詳細は書かせない)。クラスタリング(重複統合)を要約より先に行うことで実現している。関連報道がない場合は従来通りの簡潔な要約になる。

## メール本文

日次・週次いずれも、対象期間にe-Gov由来の法令改正情報があれば、件数リンクとは別に**タイトル・要約・リンクをメール本文に直接列挙**する([src/digest.ts](src/digest.ts) `buildLawAmendmentSection`)。それ以外の一般トピックは従来通りダッシュボードへのリンクのみ。
...
src/relevance.ts  Claude APIによる関連性フィルタ (未設定時はキーワード一致のみ)
